...
export default function OnlinePMES() {
  ...
  const handleReserveSeat = async (sessionId: string) => {
    setReserving(sessionId);
    setMessage(null);

    try {
      const response = await fetch(`/api/pmes/${sessionId}/reserve`, {
        method: 'POST',
      });
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: data.error || 'Failed to reserve seat' });
      }
    } catch (error) {
      console.error('Error reserving seat:', error);
      setMessage({ type: 'error', text: 'Failed to reserve seat. Please try again.' });
    } finally {
      setReserving(null);
    }
  };
          ...
          <div className="space-y-8">
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600">Check back later for new PMES sessions.</p>
              </div>
            ) : (
              <div className="grid md:grid-cols-2 gap-8">
                {upcomingSessions.map((session) => (
                  <div key={session.id} className="bg-white/60 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/30 hover:bg-white/70 transition-all duration-300">
                    ...
                    <button
                      onClick={() => handleReserveSeat(session.id)}
                      disabled={reserving === session.id || session.registered >= session.capacity}
                      className={`w-full py-3 px-6 rounded-2xl font-semibold transition-all duration-300 transform hover:-translate-y-1 flex items-center justify-center group ${
