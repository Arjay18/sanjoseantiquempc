...
export default function GuimarasBranchDashboard() {
  ...
  useEffect(() => {
    ...
    if (!session || session.user?.role !== 'branch' || (session.user as any)?.branch !== 'guimaras') {
      ...
      router.replace('/branch/guimaras/login');
      return;
    }
  ...
  // Filter applications for Guimaras branch only
  const branchApplications = applications.filter(app => (app as any).branch === 'guimaras');
