      ...
      <div className="p-5">
        ...
        <div className="mt-4 space-y-2">
          <div className="rounded-xl bg-[#F8F9FA] border border-[#E5E7EB] p-3">
            ...
            <div className="text-sm font-bold text-[#004D2D]">{product.minimumInitialDeposit}</div>
          </div>
          <div className="rounded-xl bg-[#F8F9FA] border border-[#E5E7EB] p-3">
            <div className="text-xs font-semibold uppercase tracking-wide text-gray-500">Dividend Information</div>
            <div className="text-sm font-bold text-[#004D2D]">{product.dividendInformation}</div>
          </div>
        </div>
        ...
        <ul className="mt-4 space-y-2">
          {product.keyBenefits.slice(0, 3).map((b) => (
            <li key={b} className="flex items-start gap-2 text-sm text-gray-700">
              <CheckCircle2 className="w-4 h-4 text-[var(--sjmpc-green)] mt-0.5 flex-shrink-0" aria-hidden="true" />
              <span>{b}</span>
            </li>
          ))}
        </ul>
